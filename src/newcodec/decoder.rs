...
fn decode_property(
    ...
    match property_type {
        PropertyType::PayloadFormatIndicator => {
            ...
            Ok(Some(Property::PayloadFormatIndicator(
                PayloadFormatIndicator(format_indicator),
            )))
        }
        PropertyType::MessageExpiryInterval => {
            let message_expiry_interval = read_u32!(bytes);
            Ok(Some(Property::MessageExpiryInterval(
        ...
        PropertyType::ReceiveMaximum => {
            ...
            Ok(Some(Property::ReceiveMaximum(ReceiveMaximum(
                receive_maximum,
            ))))
        }
        PropertyType::TopicAliasMaximum => {
            let topic_alias_maximum = read_u16!(bytes);
            Ok(Some(Property::TopicAliasMaximum(TopicAliasMaximum(
                topic_alias_maximum,
            ))))
        }
        ...
        PropertyType::RetainAvailable => {
            ...
            Ok(Some(Property::RetainAvailable(RetainAvailable(
                retain_available,
            ))))
        }
...
mod tests {
    ...
    fn test_decode_subscribe() {
        ...
        let decoded = decode_mqtt(&mut packet, ProtocolVersion::V500)
            .unwrap()
            .unwrap();
